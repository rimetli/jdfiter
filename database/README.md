...
本项目使用 MySQL 8，数据库结构以 Alembic 迁移作为唯一事实来源。为方便部署和审计，仓库同时提供了与当前迁移对应的[完整建表语句](完整建表语句.md)快照。
...
以下两种方式任选其一；不要在同一个空库中重复执行。

### 方式 A：执行完整 SQL 快照

将[完整建表语句](完整建表语句.md)中的 SQL 代码块保存为 `schema.sql` 后执行。该方式会创建全部表、索引、外键，并写入当前 Alembic 版本。

### 方式 B：通过 Alembic 迁移（推荐）
